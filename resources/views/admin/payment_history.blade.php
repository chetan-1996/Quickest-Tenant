            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <table id="payment-status-datatable"
                            class="table table-centered table-striped table-sm nowrap w-100">
...
{{--                                    <th>Card Name</th>--}}
        ...
        <!-- end row-->
        <div id="payment-status-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-right" style="width: 100%;">
                <div class="modal-content" style="height: 100%;">
                    ...
                    <div class="modal-body">
                        <form class="ps-3 pe-3 payment-status-form" id="payment-status-form" action="#">
                            @csrf
                            <div class="form-floating mb-3">
                                <input class="form-control bg-light text-dark" type="text" id="code" name="code"
                                    required="" placeholder="Enter Payment History" autofocus>
                                <label for="code" class="form-label">Payment History <span
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            "use strict";
            var table = $("#payment-status-datatable").DataTable({
                // dom: 'Bfrtip',
                ...
                ajax: {
                    url: "{{ route('admin.payment_history.index') }}",
                    data: function(d) {
                   ...
                   /* {
                    ...
                    },*/
                    {
                        ...
                        render: function(data, type, row) {
                            if (data == 0) {
                                return '<span class="badge badge-warning-lighten">Pending</span>';
                            } else if (data == 1) {
            ...
            table.buttons().container().appendTo("#payment-status-datatable_wrapper .col-md-6:eq(0)"), $(
            ...
            formValition('#payment-status-form');
            $('.payment-status-form').on('submit', function(e) {
                ...
                if ($(this).parsley().isValid()) {
                    $.ajax({
                        async: false,
                        type: 'POST',
                        url: '{{ route('admin.payment_history.store') }}',
                        contentType: false,
                        cache: false,
                        processData: false,
                        ...
                        success: function(data) {
                            ...
                            $('#payment-status-modal').modal('toggle');
                            table.ajax.reload();
        ...
        function edit_id(id) {
            $.ajax({
                async: false,
                type: "GET",
                url: "{{ route('admin.payment_history.show') }}",
                data: {
                    id: id
                },
                dataType: "json",
                success: function(res) {
                    resetFormValidation("#payment-status-form");
                    $('#id').val(res.data.id);
                    ...
                    $('.modal-title').text('Edit Payment History');
                    $('#payment-status-modal').modal('toggle');
                }
            });
        }
        ...
        $('.delete_all').on('click', function(e) {
            ...
            remove_id(join_selected_values, '{{ route('admin.payment_history.delete') }}',
        ...
        $('.active_status_all').on('click', function(e) {
            ...
            var join_selected_values = allVals.join(",");
            change_status(join_selected_values, 0, '{{ route('admin.payment_history.edit-status') }}',
                '#payment-status-datatable');
        });
        ...
        $('.deactive_status_all').on('click', function(e) {
            ...
            var join_selected_values = allVals.join(",");
            change_status(join_selected_values, 1, '{{ route('admin.payment_history.edit-status') }}',
                '#payment-status-datatable');
        });
    </script>
@endpush
