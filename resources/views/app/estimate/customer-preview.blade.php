            ...
            <div class="row justify-content-center">
                <div class="col-xxl-12 col-lg-12">
                    <div class="card">
                        ...
                        <div class="p-4 card-header">
                            ...
                            <span class="float-end">
                                ...
                                @else
                                    ...
                                    {!! ($data['status']=='Accept')? "<span class='badge badge-outline-success bg-success-lighten'>Accepted":"<span class='badge badge-outline-danger bg-danger-lighten'>Declined</span>" !!}
                                    </div>
                                @endif
                            </span>
                        ...
                        @php
                            $new_pdf_name = ($data['estimate_version'] > 0)?$data['estimate_no'].'-V'.$data['estimate_version'] :$data['estimate_no'];
                        @endphp
                        <div class="card-body p-2 text-center">
                            {{--<object
                                data='{{url('/'). Storage::disk('s3')->url('document/'.$data['company_id'].'/' . $new_pdf_name . '.pdf')}}'
                            ...
                            >
                                ...
                                <iframe
                                    src='{{url('/').Storage::url('document/'.$data['company_id'].'/' . $new_pdf_name . '.pdf')}}'
                            ...
                            <object
                                data='{{Storage::disk('s3')->temporaryUrl(trim('public/'.$data["company_id"].'/documents/' . $new_pdf_name . '.pdf'),Carbon\Carbon::now()->addMinutes(20))}}'
                                type="application/pdf"
                                style="width: 100%"
                                height="678"
                            >
                                ...
                                <iframe
                                    src='{{Storage::disk('s3')->temporaryUrl(trim('public/'.$data["company_id"].'/documents/' . $new_pdf_name . '.pdf'),Carbon\Carbon::now()->addMinutes(20))}}'
