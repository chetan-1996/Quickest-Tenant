...
@section('content')
    <div class="content-page">
        <div class="content">
            ...
            {{--                    @if(in_array('add-lead-origin', $user_perm) || auth()->user()->company_id==null)--}}
                                <a href="javascript:void(0);" class="btn btn-primary btn-sm mb-2"
                                   onclick="openModal('#customer-lead-modal','Create Customer Lead','#customer-lead-form','.modal-title',id=0)"><i
            ...
            {{--                    @endif--}}
                            ...
                            <div class="page-title-left pt-2">
                                ...
                                <select class="form-select" id="fil_status" name="fil_status"
                                    ...
                                    <option value="0">Active Lead Sources</option>
                                    <option value="1">Deactive Lead Sources</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <table id="customer-lead-datatable" class="table table-centered table-sm w-100 nowrap">
                ...
                <!-- end row-->
                <div id="customer-lead-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog modal-lg modal-right" style="width: 100%;">
                        <div class="modal-content" style="height: 100%;">
                            <div class="modal-header border-1 bg-light">
                                <h4 class="modal-title">Create Customer Lead</h4>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                <form class="ps-3 pe-3 customer-lead-form" id="customer-lead-form" action="#">
                                    ...
                                    <div class="text-end">
                                        ...
                                        <button class="btn btn-primary btn-sm" id="lead_button" type="submit"><i
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            "use strict";
            var table = $("#customer-lead-datatable").DataTable({
                // dom: 'Bfrtip',
                dom:
                ...
                ajax: {
                    url: "{{ route('tenant.customer-lead.index', ['tenant' => $segment]) }}",
                    data: function (d) {
                ...
                columns: [
                    ...
                    {data: 'description', name: 'description'},
                    {
                        ...
                        render: function (data, type, row) {
                            if(row.is_status==0) {
                                var fun_status = "change_status('" + row.action + "', 1,'{{route('tenant.customer-lead.edit-status', ['tenant' => $segment])}}','#customer-lead-datatable')";
                                if (data == 0)
                                    return '<span class="badge badge-success-lighten" onclick="' + fun_status + '">Active</span>';
                                else {
                                    fun_status = "change_status('" + row.action + "', 0,'{{route('tenant.customer-lead.edit-status', ['tenant' => $segment])}}','#customer-lead-datatable')";
...
return '';
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            var delete_fun = "remove_id('" + row.action + "','{{route('tenant.customer-lead.delete', ['tenant' => $segment])}}','#customer-lead-datatable')";
                            if(row.is_status==0) {
                                return '<div class="invoice-action">' +
                                    {{--                                @if(in_array('edit-lead-origin', $user_perm) || auth()->user()->company_id==null)--}}
                                        '<a href="javascript:void(0)" class="action-icon mr-1" id="edit_' + row.action + '" onclick="' + edit_fun + '">' +
                                    '<i class="mdi mdi-square-edit-outline"></i>' +
                                    '</a>' +
                                    {{--                                @endif--}}
                                        {{--                                @if(in_array('remove-lead-origin', $user_perm) || auth()->user()->company_id==null)--}}
            ...
            table.buttons().container().appendTo("#customer-lead-datatable_wrapper .col-md-6:eq(0)"), $("#alternative-page-datatable").DataTable({
            ...
            formValition('#customer-lead-form');
            $('.customer-lead-form').on('submit', function (e) {
                e.preventDefault();
                if ($(this).parsley().isValid()) {
                    $.ajax({
                        async: false,
                        type: 'POST',
                        url: '{{route('tenant.customer-lead.store', ['tenant' => $segment])}}',
                        contentType: false,
                        cache: false,
                        processData: false,
                        ...
                        dataType: "json",
                        beforeSend: function () {
                            $("#customer-lead_button").prop('disabled', true);
                            $("#customer-lead_button").html('<i class="mdi mdi-spin mdi-loading"></i> Loading...');
                        },
                        success: function (data) {
                            toastrSuccess('Successfully saved...', 'Success');
                            $('#customer-lead-modal').modal('toggle');
                            table.ajax.reload();
                            $("#customer-lead_button").prop('disabled', false);
                            $("#customer-lead_button").html('<i class="uil-arrow-circle-right"></i> Save');
                        },
                        error: function (xhr, status, error) {
                            ...
                            switch (xhr.status) {
                                case 401:
                                    ...
                                    break;
                                case 422:
                                    toastrInfo('The customer-lead is invalid.', 'Info');
                                    break;
                                case 409:
                        ...
                        complete: function (data) {
                            $("#customer-lead_button").html('Save');
                            $("#customer-lead_button").prop('<i class="uil-arrow-circle-right"></i> disabled', false);
                        }
                    });
                }
            });
        ...
        function edit_id(id) {
            $.ajax({
                async: false,
                type: "GET",
                url: "{{route('tenant.customer-lead.show', ['tenant' => $segment])}}",
                data: {id: id},
                dataType: "json",
                success: function (res) {
                    resetFormValidation("#customer-lead-form");
                    $('#id').val(res.data.id);
                    $('#name').val(res.data.name);
                    $('#description').val(res.data.description);
                    $('.modal-title').text('Edit Customer-lead');
                    $('#customer-lead-modal').modal('toggle');
                }
            });
        }
        ...
        $('.delete_all').on('click', function (e) {
            ...
            remove_id(join_selected_values, '{{route('tenant.customer-lead.delete', ['tenant' => $segment])}}', '#customer-lead-datatable');
        });
        ...
        $('.active_status_all').on('click', function (e) {
            ...
            var join_selected_values = allVals.join(",");
            change_status(join_selected_values, 0, '{{route('tenant.customer-lead.edit-status', ['tenant' => $segment])}}', '#customer-lead-datatable');
        });
        ...
        $('.deactive_status_all').on('click', function (e) {
            ...
            var join_selected_values = allVals.join(",");
            change_status(join_selected_values, 1, '{{route('tenant.customer-lead.edit-status', ['tenant' => $segment])}}', '#customer-lead-datatable');
        });
    </script>
@endpush
