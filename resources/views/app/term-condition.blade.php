...
<div class="content-page">
        ...
        <div class="container-fluid">
                ...
                <div class="col-12">
                    <div class="page-title-box">
                        <div class="page-title-right">
                            {{--  @if(in_array('add-term-condition', $user_perm) || auth()->user()->company_id==null)--}}
                            <a href="{{route("tenant.term-condition.new", ['tenant' => $segment])}}" class="btn btn-primary btn-sm mb-2"><i class="mdi mdi-plus-circle"></i> New</a>
                            {{--  @endif--}}
                            <div class="dropdown btn-group mb-2">
                                ...
                                <div class="dropdown-menu dropdown-menu-animated">
                                    ...
                                    <a href="javascript:void(0);" class="dropdown-item deactive_status_all"><i
                                            ...
                                            {{--   @if(in_array('remove-term-condition', $user_perm) || auth()->user()->company_id==null)--}}
                        ...
                        <div class="page-title-left pt-2">
                            {{--<h4 class="page-title">Term & Condition</h4>--}}
                            <select class="form-select" id="fil_status" name="fil_status" style="width: 170px;background-color: #fff0 !important;border: 0px solid #fff !important;font-size: 18px;margin: 0;white-space: nowrap;font-weight: 700;padding: 0.0rem 0.0rem 0rem 0.5rem;">
                                ...
                                <option value="0">Active Terms</option>
                                <option value="1">Deactive Terms</option>
                            </select>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <table id="term-condition-datatable" class="table table-centered table-sm w-100 nowrap">
    ...
    <div id="term-condition-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-right" style="width: 100%;">
            <div class="modal-content" style="height: 100%;">
                <div class="modal-header border-1 bg-light">
                    <h4 class="modal-title">Create Term & Condition</h4>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            "use strict";
            var table = $("#term-condition-datatable").DataTable({
                // dom: 'Bfrtip',
                dom:
                ...
                ajax: {
                    url: "{{ route('tenant.term-condition.index', ['tenant' => $segment]) }}",
                    data: function (d) {
                ...
                columns: [
                    ...
                    {data: 'description', name: 'description',visible: false},
                    {
                        ...
                        render: function (data, type, row) {
                            var fun_status = "change_status('" + row.action + "', 1,'{{route('tenant.term-condition.edit-status', ['tenant' => $segment])}}','#term-condition-datatable')";
                            if (data == 0)
                                return '<span class="badge badge-success-lighten" onclick="' + fun_status + '">Active</span>';
                            else {
                                fun_status = "change_status('" + row.action + "', 0,'{{route('tenant.term-condition.edit-status', ['tenant' => $segment])}}','#term-condition-datatable')";
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            var edit_fun = "{{ url($segment.'/term-condition/edit') }}/" + row.action;
                            // var edit_fun = "edit_id('" + row.action + "')";
                            var delete_fun = "remove_id('" + row.action + "','{{route('tenant.term-condition.delete', ['tenant' => $segment])}}','#term-condition-datatable')";
                            return '<div class="invoice-action">' +
{{--                                @if(in_array('edit-term-condition', $user_perm) || auth()->user()->company_id==null)--}}
...
{{--                                @endif--}}
            ...
            table.buttons().container().appendTo("#term-condition-datatable_wrapper .col-md-6:eq(0)"), $("#alternative-page-datatable").DataTable({
                ...
                if ($(this).parsley().isValid()) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('#term-condition-modal').modal('toggle');
                            table.ajax.reload();
        ...
        function edit_id(id) {
            $.ajax({
                async: false,
                type: "GET",
                url: "{{route('tenant.term-condition.show', ['tenant' => $segment])}}",
                data: {id: id},
                dataType: "json",
                success: function (res) {
                    resetFormValidation("#term-condition-form");
                    $('#id').val(res.data.id);
                    $('#name').val(res.data.name);
                    $('#description').val(res.data.description);
                    CKEDITOR.instances['description'].setData(res.data.description);
                    $('.modal-title').text('Edit Term & Condition');
                    $('#term-condition-modal').modal('toggle');
                }
            });
        }
        ...
        $('.delete_all').on('click', function (e) {
            ...
            remove_id(join_selected_values, '{{route('tenant.term-condition.delete', ['tenant' => $segment])}}', '#term-condition-datatable');
        });
        ...
        $('.active_status_all').on('click', function (e) {
            ...
            var join_selected_values = allVals.join(",");
            change_status(join_selected_values, 0, '{{route('tenant.term-condition.edit-status', ['tenant' => $segment])}}', '#term-condition-datatable');
        });
        ...
        $('.deactive_status_all').on('click', function (e) {
            ...
            var join_selected_values = allVals.join(",");
            change_status(join_selected_values, 1, '{{route('tenant.term-condition.edit-status', ['tenant' => $segment])}}', '#term-condition-datatable');
        });
    </script>
@endpush
