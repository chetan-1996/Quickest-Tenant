...
@section('content')
    <div class="content-page">
        <div class="content">
            ...
            {{--                    @if(in_array('add-tax', $user_perm) || auth()->user()->company_id==null)--}}
                                <a href="javascript:void(0);" class="btn btn-primary btn-sm mb-2"
                                   onclick="openModal('#tax-modal','Create Tax (GSTIN)','#tax-form','.modal-title',id=0)"><i
            ...
            {{--                            @if(in_array('remove-tax', $user_perm) || auth()->user()->company_id==null)--}}
            ...
            {{--                            @endif--}}
                            ...
                            <div class="page-title-left pt-2">
                                ...
                                <select class="form-select" id="fil_status" name="fil_status" style="width: 180px;background-color: #fff0 !important;border: 0px solid #fff !important;font-size: 14px;margin: 0;white-space: nowrap;font-weight: 700;padding: 0.0rem 0.0rem 0rem 0.5rem;">
                                    <option value="">All Tax (GSTIN)</option>
                                    <option value="0">Active Tax (GSTIN)</option>
                                    <option value="1">Deactive Tax (GSTIN)</option>
                                </select>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <table id="tax-datatable" class="table table-centered table-sm w-100 nowrap">
                ...
                <!-- end row-->
                <div id="tax-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-right" style="width: 100%;">
            <div class="modal-content" style="height: 100%;">
                <div class="modal-header border-1 bg-light">
                    <h4 class="modal-title">Create Tax (GSTIN)</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="ps-3 pe-3 tax-form" id="tax-form" action="#">

                        <div class="mb-3">
                            <label for="name" class="form-label">Name <span class="text-danger">*</span></label>
                            <div class="input-group input-group-merge">
                                <input class="form-control" type="text" id="name" name="name" required="" placeholder="Enter name" autofocus data-parsley-errors-container="#errorBlock" data-parsley-pattern="[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)" data-parsley-pattern-message="Allow only numeric values">
                                <span class="input-group-text icon"><i class="mdi mdi-percent-outline"></i></span>
                            </div>
                        ...
                        <div class="text-end">
                            ...
                            <button class="btn btn-primary btn-sm" id="tax_button" type="submit"><i
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            "use strict";
            var table = $("#tax-datatable").DataTable({
                // dom: 'Bfrtip',
                dom:
...
{{--                                @endif--}}
            ...
            table.buttons().container().appendTo("#tax-datatable_wrapper .col-md-6:eq(0)"), $("#alternative-page-datatable").DataTable({
            ...
            $('.tax-form').on('submit', function (e) {
                ...
                if ($(this).parsley().isValid()) {
                    $.ajax({
                        ...
                        dataType: "json",
                        beforeSend: function () {
                            $("#tax_button").prop('disabled', true);
                            $("#tax_button").html('<i class="mdi mdi-spin mdi-loading"></i> Loading...');
                        },
                        success: function (data) {
                            toastrSuccess('Successfully saved...', 'Success');
                            $('#tax-modal').modal('toggle');
                            table.ajax.reload();
                            $("#tax_button").prop('disabled', false);
                            $("#tax_button").html('<i class="uil-arrow-circle-right"></i> Save');
                        },
                        ...
                        complete: function (data) {
                            $("#tax_button").html('Save');
                            $("#tax_button").prop('<i class="uil-arrow-circle-right"></i> disabled', false);
                        }
                    });
                }
            });
        ...
        function edit_id(id) {
            $.ajax({
                ...
                success: function (res) {
                    resetFormValidation("#tax-form");
                    $('#id').val(res.data.id);
                    $('#name').val(res.data.name);
                    $('.modal-title').text('Edit Tax (GSTIN)');
                    $('#tax-modal').modal('toggle');
                }
            });
        }
