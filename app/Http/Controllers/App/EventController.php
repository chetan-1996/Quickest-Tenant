...
use App\Models\{Estimate, Event, SalesPersonPerformances};
use Auth;
use Carbon\Carbon;
...
class EventController extends Controller
{
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($input['sp_flag'] == 'u') {
                Event::where('id', $input['event_id'])->update(array('read_at' => date('y-m-d H:i:s'), 'read_by' => $user->id));
    ...
    public function followUpHistoryIndexNew()
    {
        return view('app.follow-up-history-new');
    }
    ...
    public function getDateWiseFollowUpList(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (count($data) > 1 && strtotime(date("d-m-Y",strtotime($dateArr[0]))) > strtotime(date("d-m-Y",strtotime($dateArr[1])))) {
                krsort($data);
            }
            if (count($data) > 1 && strtotime(date("d-m-Y",strtotime($dateArr[0]))) < strtotime(date("d-m-Y",strtotime($dateArr[1])))) {
                ksort($data);
