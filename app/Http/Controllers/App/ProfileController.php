...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('profile_icon')) {
            ...
            $path = Storage::disk('s3')->put('public/'.$tmpId.'/profile', $request->profile_icon);
            $input['profile_icon'] = 'public/'.$tmpId.'/profile/'.basename(Storage::disk('s3')->url($path));*/
        }

        $input['status'] = 'Approved';
        // $input['mobile_no'] = $input['full'];
        $input['mobile_no'] = '+'.$input['country_code'].$input['mobile_no'];
        // unset($input['full']);
...
//                ['permission_id' => 3, 'user_id' => $tmpId, 'company_id' => $tmpId, 'is_primary' =>0],
            ...
            if ($userDatas->company_category == 1) {
                ...
                $filename_one = 'public/uploads/thumbnail/'.date('YmdHis') . "106" . ".png";
                ...
                $filename_two = 'public/uploads/thumbnail/'.date('YmdHis') . "107" . ".png";
                ...
                $filename_three = 'public/uploads/thumbnail/'.date('YmdHis') . "108" . ".png";
                ...
                Image::make($panel9T_2_path_one)->resize(64, 64)->save(storage_path("app/public/uploads/resize_image/" . $panel9T_2_one),'public');*/
                ...
                Storage::disk('s3')->put("public/".$tmpId."/products/resize_image/" . $panel29T_1_three, $image->stream()->__toString());
            ...
            if ($userDatas->company_category == 1) {
                ...
                Storage::disk('s3')->put("public/".$tmpId."/templates/aboutus/" . $filename_aboutus_one, file_get_contents($aboutus_one));
            ...
            /*$commercial_path_three = env('APP_URL')."sample/broucher/quickest-broucher.pdf";
            $commercial_three = rand(1000, 9999) . ".pdf";*/
            // Image::make(base64_decode($commercial_path_three))->save(storage_path("app/public/content/files/" . $commercial_three));

            // Storage::copy($commercial_path_three, storage_path("app/public/content/files/" . $commercial_three));
            $contentFileArr = [
                /*['name' => 'Quickest Broucher ', 'path' => 'public/content/files/quickest-broucher.pdf', 'status' => 0, 'user_id' => $tmpId, 'company_id' => $tmpId, 'created_at' => date('Y-m-d H:i:s'), 'updated_at' => date('Y-m-d H:i:s')]*/
        ...
        Mail::to($user->email)->send(new ClientUserMail($user));
    ...
    public function userAccount()
    {
        ...
        $countries = Country::select(["name", "id"])->where('status', '=', 0)->get();
        $countries_phone_code = Country::where('status', '=', 0)->select("phonecode")->distinct()->where('phonecode', '>', 0)->orderBy('phonecode','asc')->get();
        $states = State::select(["name", "id"])->where([['country_id', 101], ['status', '=', 0]])->get();
        $company_categories = CompanyCategory::select(["name", "id"])->where('status', '=', 0)->get();
        $company_data = User::select('id','email','mobile_no','country_id')->where("id", $company_id)->first();
        $company_data = User::select('users.id', 'users.email', 'users.mobile_no', 'users.country_id', 'countries.phonecode as country_phonecode')
            ->join('countries', 'users.country_id', '=', 'countries.id')
            ->where('users.id', $company_id)
            ->first();
        $segment = $this->segment;
//        dd($company_data);
        return view('app.auth.user-account', compact('user', 'countries', 'company_categories', 'states','countries_phone_code','company_data', 'segment'));
    }
    ...
    public function update_image(Request $request){
        ...
        if ($request->hasFile('profile_icon')) {
            $request->validate([
                'profile_icon' => 'image|mimes:jpeg,png,jpg,Jpeg,Png,Jpg,JPEG,PNG,JPG|max:1024',
            ]);
            // dd($request->profile_icon);
           /* if (Storage::exists($user->profile_icon)) {
                Storage::delete($user->profile_icon);
            }*/
            ...
            /*$path = $request->file('profile_icon')->store('public/profile', 's3');
            $input['profile_icon'] = Storage::disk('s3')->url($path);*/

            $path = Storage::disk('s3')->put('public/'.$company_id.'/profile', $request->profile_icon,'public');
//            $file_name = Storage::disk('s3')->setVisibility($path, 'public');

            $input['profile_icon'] = 'public/'.$company_id.'/profile/'.basename(Storage::disk('s3')->url($path));
        }
        User::find($user->id)->update($input);
        $activityLogMsg = 'Profile photo updated by ' . $user->name;
        LogActivity::addToLog($activityLogMsg, $input);
        return response()->json(['success' => 'Acoount setting Updated!', 'amw'=> $input['profile_icon']], 201);
    ...
    public function postUserAccount(Request $request)
    {
        ...
        DB::connection('mysql')->table('tenants')->where('email', $user->email)->update($input);
        User::find($user->id)->update($input);
        $activityLogMsg = 'Account setting updated by ' . $user->name;
        LogActivity::addToLog($activityLogMsg, $input);

        return response()->json(['success' => 'Acoount setting Updated!'], 201);
    }
    ...
    public function postResetPassword(Request $request)
    {
        ...
        if (!Hash::check($input['old_password'], $user->getAuthPassword())) {
            return response()->json(['errors' => 'Current password wrong!'], 409);
        }

        User::find($user->id)->update(['password' => Hash::make($input['new_password'])]);
        $activityLogMsg = 'Password updated by ' . $user->name;
        LogActivity::addToLog($activityLogMsg, $input);
