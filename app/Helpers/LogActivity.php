...
use App\Models\EstimateTimeline as EstimateTimelineModel;
use App\Models\ContentMessageTimeline as ContentMessageTimelineModel;
use App\Models\ContentFileTimeline as ContentFileTimelineModel;
...
//        dd(DB::getQueryLog());
    ...
    public static function convertToIndianCurrency($number,$currency_name=null)
    {
        ...
        $paisa = 'Cents';
        if($currency_name == 'Indian Rupee' || $currency_name == 'INR')
            $paisa = 'Paise';
        $paise = ($decimal) ? "And $paisa " . ($words[$decimal - $decimal % 10]) . " " . ($words[$decimal % 10]) : '';
        return ($Rupees ? $currency_name.' ' . $Rupees : '') . $paise . " Only";
    }
...
//        $log = [];
        ...
        EstimateTimelineModel::create($log);
    }

    public static function addToMessageActivityLog($log)
    {
        ContentMessageTimelineModel::create($log);
    }

    public static function addToFileActivityLog($log)
    {
        ContentFileTimelineModel::create($log);
    }

    public static function addToActivityLogGuest($log)
    {
        EstimateTimelineModel::create($log);
    }

    public static function addToActivityLogTenant($log)
    {
//        $log = [];
        $log['user_id'] = 1;
        $log['company_id'] = 1;
